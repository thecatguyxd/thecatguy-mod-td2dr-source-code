function scr_tails_special()
{
    ...
    if (global.playerControls && isFlying && keyboard_check_pressed(global.KeyDown))
    {
        isFlying = false
		isJumping = true
        flyTimer = 0
    ...
    if (global.playerControls && keyboard_check_pressed(global.KeyB) && attackTimer == 0 && attackCharge == 0)
    {
        ...
        var pack = cpacket_tcp((83 << 0))
        send_server_tcp(pack)
        attackTimer = (revivalTimes >= 2 ? 720 : 1380)
        attackCharge = 1
    }
    if (attackCharge > 0)
    {
        ...
        if (attackCharge >= 210 || (!global.playerControls) || isZipline || isHurt || isDead || shockedTimer > 0 || (!keyboard_check(global.KeyB)))
        {
            ...
            if (revivalTimes >= 2)
            {
                ...
                if (attackCharge >= 126)
                    dmg = 60
                net_tails_spawn_projectile(x, y, image_xscale, dmg, true, ceil(val / 20))
            }
            else
                net_tails_spawn_projectile(x, y, image_xscale, (1 + (floor(attackCharge / 42))), false, ceil(val / 20))
