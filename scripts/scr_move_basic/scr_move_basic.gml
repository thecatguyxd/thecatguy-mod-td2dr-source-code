function scr_move_basic()
{
	...
	//*new* jump taked from sonic mod sorry
	if isGrounded
    {
        isJumping = false
        if (global.playerControls && canLookDown && keyboard_check(global.KeyDown))
        {
            ...
            else if (abs(gspd) >= 2 && (!isSpinning) && canSpin)
            {
    ...
    else if (global.playerControls && keyboard_check_pressed(global.KeyA) && (!isJumping) && (!isSpindashing) && (!isLookingDown))
    {
